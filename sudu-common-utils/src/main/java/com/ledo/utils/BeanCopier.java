...
import com.ledo.conversion.TypeConverterUtils;
import com.ledo.exception.Exceptions;
import javassist.*;
...
public class BeanCopier {
    ...
    private static class Generator {
        ...
        private boolean checkCanGenSource(PropertyDescriptor setter, PropertyDescriptor getter) {
            ...
            // 检查getter是否存在
            if (getter == null) {
                log.debug("[对象属性复制]原对象[{}.{}]getter方法不存在", source.getCanonicalName(), setter.getName());
                return false;
            }
            // 检查getter的读方法是否存在
            if (getter.getReadMethod() == null) {
                log.debug("[对象属性复制]原对象[{}.{}]getter方法不存在", source.getCanonicalName(), getter.getName());
                return false;
            }
            // 检查setter的写方法是否存在
            if (setter.getWriteMethod() == null) {
                log.debug("[对象属性复制]目标对象[{}.{}]setter方法不存在", target.getCanonicalName(), setter.getName());
                return false;
            }
            return true;
        }
        ...
        public Class<Copy> generate() {
            ...
            try {
                ...
                copyClass = (Class<Copy>) cc.toClass(classLoader, null);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            return copyClass;
